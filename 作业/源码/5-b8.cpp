...
void output(const char prompt[], const char d[])
{
	...
	int last_non_zero = N_NUM;
	while (last_non_zero >= 1 && d[last_non_zero] == 0) {
		last_non_zero--;
	}
...
{
	d_next[0] = d_cur[0] - 1; // 幂次减1

	int addto = 0; //进位
	for (int i = 1; i <= N_NUM; i++) {
		int value = d_cur[i] + addto * 10;
		d_next[i] = value / 2;
		addto = value % 2;
	}
...
void calc_num(char pure_decimal[], char out[], char power_table[][N_NUM + 1])
{
	...
	// 二进制每一位
	for (int i = 1; i <= -pure_decimal[0]; i++) {
		if (pure_decimal[i] == 1) {
			// 将对应的幂次表行加到结果中
			int carry = 0;
...
void input_pure_decimal(char pure_decimal[]) {
	while (1) {
		...
		int len = 0;
		for (int i = 1; input[i] && len < N_POW; i++) {  // 修改1：合并长度检查和输入验证
			if (input[i] == '0' || input[i] == '1') {
				pure_decimal[i] = input[i] - '0';
				len++;
			}
			else {
